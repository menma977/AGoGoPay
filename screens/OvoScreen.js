...
import { View, KeyboardAvoidingView, AsyncStorage, BackHandler, Image , StatusBar } from 'react-native';
import {
    Spinner,
    Body,
    Title,
...
export default class OvoScreen extends React.Component {
    constructor ( props ) {
        super( props );
    ...
    async sendDataRequest () {
        ...
        if ( this.state.phone.length >= 10 && this.state.nominal != null ) {
            let setType = 'OVO';
            let data = await OvoController.prototype.Request( this.state.username, this.state.code, this.state.phone.replace( /-/g, '' ).replace( '+62', '0' ).replace( ' ', '' ), this.state.nominal, setType );
            if ( data.Status == 0 ) {
    ...
    async buyData () {
        ...
        let data = await OvoController.prototype.Pay( setUsername, setCode, setPhoneNumber, setPayCode, setNominal, setFirstBalance, setPrice, setRemainingBalance );
        if ( data.Status == 0 ) {
    ...
    setNominal () {
        ...
        if ( this.state.typeNumber != 7 ) {
            this.state.product[ 'OVO' ].map( function ( item, key ) {
                if ( item[ 'type' ] == 'OVO' ) {
                    data.push( { code: item[ 'code' ].replace( 'OVO ', '' ), name: item[ 'name' ] } );
                }
            } );
            return (
                            ...
                            <Header style={ { backgroundColor: '#fff',  height: 70 } } >
                                ...
                                <Body style={ { alignItems: 'flex-start' } }>
                                    <Button transparent onPress={ () => this.props.navigation.navigate( 'Home' ) } style={ { alignSelf: 'flex-start' } }>
                                        <Icon active type='Ionicons' name='ios-arrow-back' size={ 20 } style={ { color: '#ffffffff' } } />
                                    </Button>
                                </Body>
                                <Body style={ { alignItems: 'center' } }>
                                    <Title style={ { color: '#4b3854ff' } }>OVO</Title>
                                </Body>
